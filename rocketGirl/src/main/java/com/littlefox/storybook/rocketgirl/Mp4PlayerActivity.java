...
import com.littlefox.storybook.object.VibratorBaseObject;
import com.littlefox.storybook.object.VibratorObject;
...
public class Mp4PlayerActivity extends BaseActivity implements OnBufferingUpdateListener, OnCompletionListener, OnPreparedListener, OnVideoSizeChangedListener, SurfaceHolder.Callback
{
	...
	private void init()
	{
        ...
        RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(CommonUtils.getInstance(this).getPixel(446), CommonUtils.getInstance(this).getPixel(446));
	...
	{	
		...
		try
		{
			...
			if( visibleTime <= (float)mMediaPlayer.getCurrentPosition()/1000.0f)
			{
				String[] pattenList = mVibratorObject.vibrator_list.get(mVibratorPlayIndex).vibrate_patten.split(",");
				...
				Log.f("Vibartor Status : [position] = "+ mVibratorPlayIndex+", [patten] = "+ mVibratorObject.vibrator_list.get(mVibratorPlayIndex).vibrate_patten);
